...
前端已开建：[`workbench/`](workbench/) —— S01 数据契约、S02 FlowNode 组件、S03 画布与泳道、S04 五种连线、S05 选中 / 移动 / 框选 / 撤销、S06 属性面板、S07 拖线与契约校验、S08 快速添加已完成（React + TypeScript + Vite，画布底座 `@xyflow/react`，`flow.schema.json` 接入运行时校验，视觉编码、缩放、连线几何、编辑操作、面板、拖线与添加都有真浏览器断言）。投标示例已能在画布上完整看到、拖动、改字段、删除、拖线、加节点：泳道、节点、五种线与标签、框选与撤销、右栏三态、不合规的线拖的时候就变红、左栏组件库拖入与端口拖到空白即搜即建；泳道语义从 S09 起。
