...
前端已开建：[`workbench/`](workbench/) —— S01 数据契约、S02 FlowNode 组件、S03 画布与泳道、S04 五种连线、S05 选中 / 移动 / 框选 / 撤销已完成（React + TypeScript + Vite，画布底座 `@xyflow/react`，`flow.schema.json` 接入运行时校验，视觉编码、缩放、连线几何与编辑操作都有真浏览器断言）。投标示例已能在画布上完整看到并拖动：泳道、节点、五种线与标签、框选与撤销；属性面板从 S06 起。
