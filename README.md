...
前端已开建：[`workbench/`](workbench/) —— S01 数据契约、S02 FlowNode 组件、S03 画布与泳道、S04 五种连线、S05 选中 / 移动 / 框选 / 撤销、S06 属性面板、S07 拖线与契约校验、S08 快速添加、S09 泳道语义与角色切换已完成（React + TypeScript + Vite，画布底座 `@xyflow/react`，`flow.schema.json` 接入运行时校验，视觉编码、缩放、连线几何、编辑操作、面板、拖线、添加与泳道语义都有真浏览器断言）。编辑核心已齐：投标示例能在画布上完整看到、拖动、改字段、删除、拖线、加节点，把人的步骤拖进 AI 泳道就标为要自动化，AI 节点拖进人泳道就改人审；对照现状与试跑从 S10 起。
