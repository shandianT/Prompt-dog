...
前端已开建：[`workbench/`](workbench/) —— S01 数据契约、S02 FlowNode 组件、S03 画布与泳道、S04 五种连线、S05 选中 / 移动 / 框选 / 撤销、S06 属性面板已完成（React + TypeScript + Vite，画布底座 `@xyflow/react`，`flow.schema.json` 接入运行时校验，视觉编码、缩放、连线几何、编辑操作与面板都有真浏览器断言）。投标示例已能在画布上完整看到、拖动、改字段、删除：泳道、节点、五种线与标签、框选与撤销、右栏诊断 / 节点 / 连线三态；连线与端口校验从 S07 起。
