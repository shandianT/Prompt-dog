...
| L3 | 提示链设计（环节表 + 数据契约 + 验证点 + 失败回退 + 内嵌闭环）；重复使用场景 + 有文件系统 → 资产包 | 提示链 + 验收标准先行 | S3=2 → 对抗审查，含可查证事实且有工具 → 交叉核查；S4≥1 → ReAct；有不可逆动作 → 该动作前设人工确认点 |
| L4 | 完整编排（子 agent 分配表 + skill 清单 + 工具清单 + 主控循环与终止）；重复使用场景 + 有文件系统 → 资产包 | 多智能体 + 提示链 + 自我修正 | 探索型目标 → ToT；长期运营 → 文档化 + APE；有不可逆动作 → 该动作前设人工确认点 |
