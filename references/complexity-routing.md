...
- **S3 准确性风险**：0 = 错了改一下就行（草稿、灵感）；1 = 错了浪费别人时间（团队周报、内部分析）；2 = 错了有实际损失（对外发布、供决策、合规文书）。用途不明（自用还是给别人看）时取 1 分并用假设对冲（「H 错误代价按中处理，对外发布请告知」）——不默认 0 分，低估风险会跳过必要验证；「取其高」仲裁只对已知对外/供决策的场景生效。
...
- 并联只在两处合法：L4 多角色独立产出再汇总（汇总点是审查者，只打钩不重写）；流程重构打包建议里多只狗并行建包（汇总点是流程图 JSON）。其余一律串联，仲裁沿用「错误代价取其高」与「打钩数不增加即止损」。
- 用户覆盖：用户明确要求「简化 / 按低一级」或「就要多智能体」→ 照办，记「H0 级别由用户指定」并一句话标注该级别的已知风险，不重新辩论。
...
- 「帮我写个周报总结提示词」：S3=1（给别人看）但仍是 L1 → 不问错误代价，写「H1 错误代价：中（依据：团队周报供他人阅读）」，附 2–3 条完成前自查，零提问交付。
- 「我们投标流程是销售录线索、商务每天刷平台下载文件、售前写标书、财务报价常卡两天、领导审批后递交，哪些能交给 AI」：这是一段流程，不是一个任务 → 不判级，走流程重构模式（process-rebuild.md）先出图与诊断；打包建议里的「标书撰写」再单独判级（S1=2 S2=2 S3=2 S6=1 → L3，重复使用 → 资产包）。
- 「设计一个每月自动对账的工作流，错了要出大事，跑在 Claude Code 里」：环境、错误代价、方向都已知 → 架构级不确定项为 0，L3 也零提问直接交付；人工确认点只放在「提交对账结果」前。

## 级别 → 产出形态 → 策略装配

| 级别 | 产出形态 | 必装策略 | 按信号加装 |
|---|---|---|---|
| L1 | 优化提示词全文 + 一行使用说明 | 人设、明确性、结构化、正面引导 | 格式/风格特殊 → 少样本；S3≥1 → 附验收标准段（错误代价写 H，不成题） |
| L2 | 步骤化 SOP（每步含目的/动作/提示词片段/检查点） | 同上 + 情境启动 | 有推理步 → CoT；S6≥1 → 输出格式化 |
| L3 | 提示链设计（环节表 + 数据契约 + 验证点 + 失败回退 + 内嵌闭环）；重复使用场景 + 有文件系统 → 资产包；会无人运行 → 加 `验收清单.json` + 上岗循环（run-loop.md） | 提示链 + 验收标准先行 | S3=2 → 对抗审查，含可查证事实且有工具 → 交叉核查；S4≥1 → ReAct；有不可逆动作 → 该动作前设人工确认点 |
| L4 | 完整编排（子 agent 分配表 + skill 清单 + 工具清单 + 主控循环与终止）；重复使用场景 + 有文件系统 → 资产包；会无人运行 → 同 L3 | 多智能体 + 提示链 + 自我修正 | 探索型目标 → ToT；长期运营 → 文档化 + APE；有不可逆动作 → 该动作前设人工确认点 |
