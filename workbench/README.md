...
流程画布 MVP 的前端工程。**已完成 S01 骨架与数据契约、S02 FlowNode 组件、S03 画布与泳道、S04 五种连线与标签、S05 选中 / 移动 / 框选 / 撤销、S06 属性面板、S07 拖线与契约校验、S08 快速添加**；泳道语义从 S09 起。
...
`scripts/check-guards.ts` 用十个故意改坏的流程图证明这四道检查真的会报错，另有六条连线判定（什么能连、连出来是什么线）。
...
├── Inspector.tsx / .css   属性面板：诊断 / 节点 / 连线三态，分类型字段，契约提示，删除
...
└── CanvasPage.tsx         07 流程画布：顶栏五个数字、缩放读数、「适应」「撤销」「产物」+ 左栏组件库 + 画布、浮条、图例、弹层、提示 + 右栏属性面板（S03–S08）
```

S05 的操作约定（SPEC §6.4）：拖节点 = 移动，选中的一起动；空白处拖 = 框选，相交即选中；Shift / Cmd / Ctrl + 点 = 加选；≥ 2 个选中出浮条。平移用按住空格拖、中键或右键拖，滚轮仍是缩放（S03 定的，等真人用过再议）。节点**锁在自己的泳道里**：跨泳道的语义（人 → AI = 要自动化并标缺口、AI → 人 = 改人审、不可逆前的人定受保护）是 S09 的事，S05 先不让跨，拖完的图仍过 x-compat 规则 2。撤销栈 40 步（Cmd/Ctrl+Z 或顶栏「撤销」），一次拖动 = 一步，只回节点与连线不回选中；方向键微移 1px、Shift 10px；Delete / Backspace 删除选中。
...
S08 的快速添加（SPEC §6.4、§6.5）：端口拖到空白松手弹组件搜索，占位符写着「接在「X」之后…」，选中即新建并连上——两个动作加一个连好线的节点；双击空白弹同一个搜索；左栏组件库五组可拖入，幽灵节点跟着指针，画布上松手即建。新节点的默认值在 `palette.ts`：数据类默认待打通，人带角色，工作狗带子图；落点按种类归入对应泳道（数据 → 数据泳道、人 → 人泳道、其余 → AI），跨泳道的语义是 S09 的事。组件库数据来自 `sample/组件库.json`，与流程示例一样从 `flow.py` 导出。
...
`sample/投标流程.json` 与 `sample/组件库.json` 由 `design/workbench/export_flow_json.py` 从 `flow.py` 的 `NODES` / `EDGES` / `PALETTE` 导出——
...
S09 泳道语义与角色切换 → S10 对照现状与试跑 → S11 打包与钻取 …
...
对快速添加的断言：端口拖到空白弹层且提示接在谁之后、输入即过滤、选中后节点 +1 线 +1 且新节点落在松手点、离端口太近不弹、双击弹层、选「人审」落进人泳道、Esc / 点空白关弹层、拖 CRM 有幽灵且落进数据泳道并待打通、拖工作狗带 7 环节、拖到画布外取消、组件库搜索、新组件占位缺口。设 `UI_SHOT_DIR=目录` 会把关键时刻的截图存下来当证据。
