...
流程画布 MVP 的前端工程。**已完成 S01 骨架与数据契约、S02 FlowNode 组件、S03 画布与泳道、S04 五种连线与标签、S05 选中 / 移动 / 框选 / 撤销、S06 属性面板、S07 拖线与契约校验**；快速添加从 S08 起。
...
└── CanvasPage.tsx         07 流程画布：顶栏五个数字、缩放读数、「适应」「撤销」「产物」+ 画布、浮条、图例 + 右栏属性面板（S03–S06）
```
...
S07 的拖线（SPEC §6.4）：从输出端口拖到另一个节点身上任何地方松手就连上；新线的种类与产物由 `connect.ts` 定——两端有数据节点是数据线，那个数据节点还在待打通就是待打通线，否则顺序线，产物 = 来源节点的输出类型。连不上的只有三种：连到自己、已经连过、决定类产物直接进数据节点（契约规则 1）；拖的过程中线就变红、原因跟着线，松手不连并在底部提示。每块卡片一直铺着一个看不见的目标 Handle，拖线中才接指针事件：React Flow 只在节点挂载时量 Handle 的位置，临时出现的 Handle 它认不出来。拖到空白松手什么都不发生，弹组件搜索是 S08。
...
对面板的断言：诊断三个数字与顶栏一致、chip 计数与点 chip 选中、四类节点各自的字段、受保护节点灰掉两个选项、改角色 / 状态 / 名称各自可撤销、**待打通改数据变实线**、产物改成决定进数据节点出契约提示、打通方式改接口的连锁（线降级、数字、标记）、删除三条路各自可撤销、关闭、重置。对拖线的断言：n8 → n13 线数 +1 且新线是顺序 / 结构化产物、悬停时线不红、松手提示、人（决定）拖到数据节点**线变红且原因跟着线、松手不连**、已经连过、连到自己、待打通数据节点出去是待打通线、拖到空白琥珀橡皮筋松手无事。设 `UI_SHOT_DIR=目录` 会把关键时刻的截图存下来当证据。
