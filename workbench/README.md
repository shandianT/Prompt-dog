...
流程画布 MVP 的前端工程。**已完成 S01 骨架与数据契约、S02 FlowNode 组件、S03 画布与泳道、S04 五种连线与标签、S05 选中 / 移动 / 框选 / 撤销**；属性面板从 S06 起。
...
├── SelectionBar.tsx       浮条：已选 N 个 · 取消（打包 S11、删除 S06 到时再加）
...
└── CanvasPage.tsx         07 流程画布第一版：顶栏五个数字、缩放读数、「适应」「撤销」「产物」+ 画布、浮条与图例（S03–S05）
```

S05 的操作约定（SPEC §6.4）：拖节点 = 移动，选中的一起动；空白处拖 = 框选，相交即选中；Shift / Cmd / Ctrl + 点 = 加选；≥ 2 个选中出浮条。平移用按住空格拖、中键或右键拖，滚轮仍是缩放（S03 定的，等真人用过再议）。节点**锁在自己的泳道里**：跨泳道的语义（人 → AI = 要自动化并标缺口、AI → 人 = 改人审、不可逆前的人定受保护）是 S09 的事，S05 先不让跨，拖完的图仍过 x-compat 规则 2。撤销栈 40 步（Cmd/Ctrl+Z 或顶栏「撤销」），一次拖动 = 一步，只回节点与连线不回选中；方向键微移 1px、Shift 10px；Delete 先不删（S06）。
...
对编辑的断言：拖 n9 (+60, +40) 写回取整、拖出画布 / 泳道被锁在 (0, 656)、撤销两步回原位、栈空再撤销无事、框选过程中有琥珀虚线框、框选 n3 / n5 / n6 出浮条、多选一起动、浮条「取消」、方向键 1px / Shift 10px、Esc、Ctrl+Z、**拖 41 次撤销 40 次回到第 1 次拖完的位置**（最早那步被挤出栈）。设 `UI_SHOT_DIR=目录` 会把关键时刻的截图存下来当证据。
