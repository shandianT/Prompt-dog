...
流程画布 MVP 的前端工程。**已完成 S01 骨架与数据契约、S02 FlowNode 组件、S03 画布与泳道、S04 五种连线与标签、S05 选中 / 移动 / 框选 / 撤销、S06 属性面板**；连线与端口校验从 S07 起。
...
S07 连线与端口类型校验 → S08 快速添加 → S09 泳道语义 …
...
对面板的断言：诊断三个数字与顶栏一致、chip 计数与点 chip 选中、四类节点各自的字段、受保护节点灰掉两个选项、改角色 / 状态 / 名称各自可撤销、**待打通改数据变实线**、产物改成决定进数据节点出契约提示、打通方式改接口的连锁（线降级、数字、标记）、删除三条路各自可撤销、关闭、重置。设 `UI_SHOT_DIR=目录` 会把关键时刻的截图存下来当证据。
