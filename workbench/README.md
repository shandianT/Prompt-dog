...
流程画布 MVP 的前端工程。**已完成 S01 骨架与数据契约、S02 FlowNode 组件、S03 画布与泳道、S04 五种连线与标签、S05 选中 / 移动 / 框选 / 撤销、S06 属性面板、S07 拖线与契约校验、S08 快速添加、S09 泳道语义与角色切换**；对照现状与试跑从 S10 起。
...
`scripts/check-guards.ts` 用十个故意改坏的流程图证明这四道检查真的会报错，另有六条连线判定（什么能连、连出来是什么线）和五条泳道语义（跨过去意味着什么、撞边说什么）。
...
└── CanvasPage.tsx         07 流程画布：顶栏五个数字、缩放读数、「适应」「撤销」「产物」+ 左栏组件库 + 画布、浮条、图例、弹层、提示、右键菜单 + 右栏属性面板（S03–S09）
```
...
S10 对照现状与试跑 → S11 打包与钻取 → S12 导入 JSON …
...
对泳道语义的断言：n14 拖进 AI 泳道**变技能、自动、标缺口**且提示、五个数字跟着变、面板说明记着原：人做、撤销位置与语义一起回；n6 拖进人泳道改人审；受保护的 n12 挡在人泳道顶并提示；数据节点挡在数据泳道顶；步骤节点挡在人泳道底；徽章循环与受保护；右键菜单八项、三种改动各自可撤销、Esc 关、受保护灰两项、框选三个右键「删除所选（3）」。设 `UI_SHOT_DIR=目录` 会把关键时刻的截图存下来当证据。
