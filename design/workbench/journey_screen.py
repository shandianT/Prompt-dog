...
PHASES = [
    ...
    ("画出现状", "30 分钟", [
        ...
        ("2", "说一段流程 + 贴材料", "01", "谁做什么、卡在哪；把文档聊天记录拖进来", "有材料反推现状并记来源，没材料按行业库出草稿标 H", None),
        ("3", "读图", "07 / 08", "只是看。三十秒答五问", "无", None),
        ("4", "纠错", "07", "说节点编号，或把步骤拖过泳道边界", "跨泳道即改「谁来做」；不可逆前的人定节点受保护", None),
    ]),
    ("建第一只狗", "30 分钟", [
        ("5", "按这个建", "07 → 02", "点主按钮，或说「先建标书撰写」", "取打包建议第一只转判级；缺口没解决也能建", "停 · 无人时取楔子 ★"),
        ("6", "确认卡", "02", "可以什么都不做；要改回「1C」", "最多两题架构级，其余转 H；产物同时在生成", None),
        ("7", "交付与入犬舍", "03 → 04", "点「入犬舍」", "资产包落盘，在画布上占一个节点并记服务流程节点号", None),
    ]),
    ("跑起来，回到图上", "1 小时 + 25 分钟", [
        ("8", "上岗一次", "05", "用一份真实材料跑启动指令", "一轮一环节，验收打钩写回 验收清单.json", None),
        ("9", "人工确认点", "05", "通过 / 退回某环节 / 先看", "对外发送前暂停，持久化等你", "停 · 不回复就一直停"),
        ("10", "复盘", "10", "说「复盘」", "读日志不重跑；规律进日志顶部；回归没过不许发布", "停 · 改版 / 不改版"),
        ("11", "回到图上", "07 / 08", "切「对照现状」，定下一只建谁", "标出原来怎么做，卡点 2 → 0", None),
    ]),
]
...
JOURNEY = HEAD + frame(COLS2, sidebar("dog") +
    topbar("用户怎么用 · 每一步", chips=("12 步", "3 个停机点", "示例：投标流程"),
           right=f'<span style="display:inline-flex;align-items:center;gap:6px;height:36px;padding:0 12px;border-radius:8px;color:{SUB};font-size:13px;white-space:nowrap">{ico("book",17,SUB)}逐步规格见 交互流程.md</span>') +
    journey_main) + TAIL
